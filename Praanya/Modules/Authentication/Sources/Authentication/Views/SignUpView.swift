...
public struct SignUpForm: View {
    ...
    public var body: some View {
        VStack(alignment: .center, spacing: 0) {
            Spacer()
            Text("CREATE ACCOUNT").kerning(4).font(.system(size: 32, weight: .light))
                .padding(.bottom, 35)
            Text("Enter your details below to create an account").foregroundColor(Color(.systemGray)).font(.system(size: 14))
                .padding(.bottom, 25)
            VStack(spacing: 15) {
