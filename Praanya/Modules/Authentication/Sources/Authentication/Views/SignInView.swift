...
public struct LoginForm: View {
    ...
    public var body: some View {
        VStack(alignment: .center, spacing: 0) {
            Spacer()
            Text("PRAANYA").kerning(10).font(.system(size: 32, weight: .light))
                .padding(.bottom, 35)
            Text("Log in below or sign up to create an account").foregroundColor(Color(.systemGray)).font(.system(size: 14))
                .padding(.bottom, 25)
            HStack(spacing: 15) {
            ...
            Button("Forgot password?") { currentPage = .forgotPassword }
                .buttonStyle(.plain).font(.system(size: 14)).foregroundColor(Color(.systemGray))
                .padding(.top, 15)
            
            Spacer()
            HStack {
                Text("Don't have an account?").foregroundColor(.black)
                Button("Sign up") { currentPage = .signUp }
                    .buttonStyle(.plain)
            }.padding(.bottom, 20)
        }
